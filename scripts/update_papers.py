...
为了更准确地判断论文是否属于“社交推荐（Social Recommendation）”领域，请参考以下背景知识：

社交推荐是指在推荐系统中显式利用社交关系、信任关系、好友关系、社交网络结构、社会影响、社群结构或用户之间的交互关系来提升推荐效果的研究方向。

典型特征包括但不限于：
1. 利用社交网络图（social graph）或信任网络进行建模；
2. 建模用户之间的社会影响（social influence）；
3. 利用好友关系或社区结构改进推荐结果；
4. trust-aware recommendation、friend-based recommendation；
5. 基于社交关系的图神经网络（Graph Neural Network for Social Recommendation）。

不属于社交推荐的情况包括：
- 普通协同过滤（未涉及社交关系）；
- 仅讨论社交网络分析但不涉及推荐；
- 单纯的内容推荐或序列推荐（未涉及社交结构）。

如果论文明确利用社交关系或社会结构信息来改进推荐系统，请将其归类为“社交推荐”。
"""
        prompt = f"""
请对以下学术论文进行分类。论文信息：
...
请判断该论文是否属于社交推荐（Social Recommendation）领域。
...
不要输出多余信息。
因为后续处理流程会使用 line.split('：')[1].strip()=='是' 来判断是否属于社交推荐。
"""
